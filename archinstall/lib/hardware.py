...
class GfxPackage(Enum):
	...
	Mesa = 'mesa'
	MesaDri = 'mesa-dri'
	MesaVulkanDrivers = 'mesa-vulkan-drivers'
	...
	def gfx_packages(self) -> list[GfxPackage]:
		...
		match self:
			...
			case GfxDriver.VirtualBox:
				...
				# Originally there was only one VM driver:
...
class SysInfo:
	...
	def virtualization() -> str | None:
		try:
			...
			virt_type = str(result).strip('\r\n')
			if virt_type == 'none':
				return None
			return virt_type
		except SysCallError as err:
			# systemd-detect-virt returns exit code 1 when not in VM (with output "none")
			# This is normal behavior, not an error
			if err.worker_log:
				output = err.worker_log.decode().strip()
				debug(f'Virtualization detected: {output}; is VM: False')
	...
	def is_vm() -> bool:
		try:
			...
			is_virtual = output != 'none'
			debug(f'Virtualization detected: {output}; is VM: {is_virtual}')
			return is_virtual
		except SysCallError as err:
			# systemd-detect-virt returns exit code 1 when not in VM (with output "none")
			# Parse the output even on "error" exit code
			if err.worker_log:
				output = err.worker_log.decode().strip().lower()
				is_virtual = output != 'none'
				debug(f'Virtualization detected: {output}; is VM: {is_virtual}')
				return is_virtual
			else:
