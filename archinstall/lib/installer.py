...
class Installer:
	...
	def set_graphics_driver(self, gfx_driver: GfxDriver) -> None:
		...
		# Schedule environment variables to be written after base installation
		self._schedule_hardware_environment()

	def _schedule_hardware_environment(self) -> None:
		"""Schedule environment variable configuration for post-base installation."""
		def configure_environment_post_install(*args: str, **kwargs: str) -> None:
			self._configure_hardware_environment()

		self.post_base_install.append(configure_environment_post_install)
		debug('Scheduled hardware environment configuration for post-base installation')
	...
	def _configure_hardware_environment(self) -> None:
		...
		elif self._gfx_driver == GfxDriver.VMOpenSource:
			debug('Adding VM environment variables')
			env_vars['MESA_LOADER_DRIVER_OVERRIDE'] = 'zink'
			env_vars['GALLIUM_DRIVER'] = 'virpipe'
	...
	def _write_environment_file(self, env_vars: dict[str, str]) -> None:
		...
		info(f'Added {len(env_vars)} environment variables to /etc/environment')
	...
	def _get_kernel_params_partition(
		...
		# Only NoEncryption is supported
		if id_root:
			if partuuid:
