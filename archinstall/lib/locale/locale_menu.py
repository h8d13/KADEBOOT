...
class LocaleMenu(AbstractSubMenu[LocaleConfiguration]):
	...
	def _mark_defaults(self) -> None:
		"""Mark locale menu items with default values"""
		items_with_defaults = [
			'kb_layout',   # Default keyboard layout
			'sys_lang',    # Default system language  
			'sys_enc',     # Default system encoding
		]
	...
	def has_modifications(self) -> bool:
		...
		return (
			self._item_group.find_by_key('kb_layout')._value_modified or
			self._item_group.find_by_key('sys_lang')._value_modified or
			self._item_group.find_by_key('sys_enc')._value_modified
		)
