...
class DiskEncryption:
	...
	def __post_init__(self) -> None:
		if self.encryption_type == EncryptionType.NoEncryption and not self.partitions:
			pass  # No encryption, no validation needed

	def should_generate_encryption_file(self, dev: PartitionModification) -> bool:
	...
	def validate_enc(
		...
		# Simplified validation without LVM
		return True
	...
	def parse_arg(
		...
		if not cls.validate_enc(disk_config.device_modifications):
			return None
