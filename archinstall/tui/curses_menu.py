...
class SelectMenu[ValueT](AbstractCurses[ValueT]):
	...
	def _item_to_vp_entry(self, items: list[list[MenuItem]]) -> list[ViewportEntry]:
		...
		for row_idx, row in enumerate(items):
			...
			for col_idx, cell in enumerate(row):
				...
				# Use new colored checkmark method
				start_pos = cur_pos
				item_entries, cur_pos = self._menu_item_entries(cell, row_idx, cur_pos, style)
				entries += item_entries
				item_text_len = cur_pos - start_pos

				if col_idx < len(row) - 1:
					spacer_len = cols_widths[col_idx] - item_text_len
	...
	def _menu_item_entries(self, item: MenuItem, row_idx: int, cur_pos: int, style: STYLE) -> tuple[list[ViewportEntry], int]:
		"""
		Create ViewportEntry objects for a menu item, handling colored checkmarks
		Returns (entries, new_cur_pos)
		"""
		entries = []
		
		if self._multi and not item.is_empty():
			prefix = self._multi_prefix(item)
			entries.append(ViewportEntry(prefix, row_idx, cur_pos, style))
			cur_pos += len(prefix)
		...
		if style_info in ['check', 'cross', 'default']:
			...
			if style_info == 'check' and Chars.Check in text:
				parts = text.rsplit(Chars.Check, 1)
				if len(parts) == 2:
					# Add text before checkmark
					entries.append(ViewportEntry(parts[0], row_idx, cur_pos, style))
					cur_pos += len(parts[0])
					# Add colored checkmark
					checkmark_style = STYLE.CHECK_MARK
				...
				if len(parts) == 2:
					...
					entries.append(ViewportEntry(text, row_idx, cur_pos, style))
					cur_pos += len(text)
			elif style_info == 'default' and Chars.Default in text:
				parts = text.rsplit(Chars.Default, 1)
				if len(parts) == 2:
					# Add text before default mark
					entries.append(ViewportEntry(parts[0], row_idx, cur_pos, style))
					cur_pos += len(parts[0])
					# Add white default mark
					default_style = STYLE.DEFAULT_MARK
					entries.append(ViewportEntry(Chars.Default, row_idx, cur_pos, default_style))
					cur_pos += len(Chars.Default)
					# Add text after default mark (if any)
					if parts[1]:
						entries.append(ViewportEntry(parts[1], row_idx, cur_pos, style))
						cur_pos += len(parts[1])
				else:
			...
			# Regular text without special styling
			entries.append(ViewportEntry(text, row_idx, cur_pos, style))
			cur_pos += len(text)
		
		return entries, cur_pos
