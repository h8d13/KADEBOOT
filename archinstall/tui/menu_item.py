...
class MenuItem:
	...
	key: str | None = None
	default_value: Any | None = field(default=None, init=False)
	_value_modified: bool = field(default=False, init=False)

	def set_value_without_marking_modified(self, value: Any) -> None:
		"""Set value without marking as modified - used during config sync"""
		self.value = value
	...
	def mark_as_modified(self) -> None:
		"""Mark value as modified from default"""
		self._value_modified = True

	def is_default_value(self) -> bool:
		"""Check if current value is the default value"""
		return not self._value_modified and self.value == self.default_value

	def has_default_and_unmodified(self) -> bool:
...
class MenuItemGroup:
	...
	def get_item_text(self, item: MenuItem) -> str:
		...
		if display_text:
			...
			if item.has_value():
				if item.has_default_and_unmodified():
